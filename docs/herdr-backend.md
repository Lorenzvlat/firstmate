...
| Pi task identity | `herdr agent get <pane>`, then `herdr agent rename <pane> <task-name>` | `agent rename` sets the human-facing AgentInfo `name` while preserving canonical `agent=pi`, rejects duplicate names with `agent_name_taken`, and does not rename the workspace or tab. Firstmate waits for exact native Pi detection and never renames another identity. |
| No Mistakes activity | `herdr pane report-metadata <pane> --source firstmate.no-mistakes ... --ttl-ms N` | One source-scoped update changes only display metadata on the owning pane. Herdr exposes no nested-agent relation, and TTL expiry plus an explicit source clear removes transient activity while preserving the owner agent, task name, workspace, and tab. |
