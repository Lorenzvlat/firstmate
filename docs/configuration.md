...
Run the creation-only `bin/fm-plan-usage-manual.sh set` helper to transcribe the bounded numeric values and UTC times from Claude's interactive `/usage` screen.
To replace or remove a snapshot, inspect and remove only the fixed `config/plan-usage-manual.json` file before the next refresh, then run `set` again only when a replacement is wanted.
An already opted-in dashboard reads a successful creation or operator removal on its next refresh without restarting Herdr or a shared daemon.
...
The shell scripts do not match those rules; firstmate chooses the best matching rule with judgment, resolves its profile object or array under the operating contract in `AGENTS.md` section 4, and passes only concrete `--harness`, `--model`, `--effort`, and fixed `--selection-reason` values to `fm-spawn.sh`.
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack an explicit harness (`--harness`, a positional adapter, or a raw launch command).
Batch spawns satisfy the same requirement with shared concrete profile and selection-reason flags.
