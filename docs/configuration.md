...
Run `bin/fm-plan-usage-manual.sh set` to transcribe the bounded numeric values and UTC times from Claude's interactive `/usage` screen, or `bin/fm-plan-usage-manual.sh clear` to remove only that snapshot.
