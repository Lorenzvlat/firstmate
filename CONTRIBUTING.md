...
- Helper scripts in `bin/` are plain bash by default; the only non-bash helpers are `bin/backends/*.py`, the `bin/*-command-policy.mjs` policy owners, and `bin/telemetry/fm-telemetry.py`, each reached from a bash entrypoint.
